...
mixin template CodeGenInstr() {
    ...
    void emitInstr(MirInstr instr) 
    {
        switch (instr.op) {
            ...
            case MirOp.GetElementPtr:
                ...
                else 
                {
                    if (auto prim = cast(PrimitiveType) baseType)
                         if (prim.baseType == BaseType.String) 
                             elemType = LLVMInt8TypeInContext(context);
                         else 
            ...
            case MirOp.ZExt:
                auto res = LLVMBuildZExt(builder, getLLVMValue(instr.operands[0]), toLLVMType(instr.dest.type), 
                    "zext");
                setReg(instr.dest, res);
                break;
            ...
            case MirOp.BOr:
                auto res = LLVMBuildOr(builder, getLLVMValue(instr.operands[0]), getLLVMValue(instr.operands[1]), 
                    "or");
                setReg(instr.dest, res);
                break;

            case MirOp.BNot:
                auto res = LLVMBuildNot(builder, getLLVMValue(instr.operands[0]), 
                    "bnot");
                setReg(instr.dest, res);
                break;
