...
class AstLowerer {
    ...
    HirProgram lower(Program ast)
    {
        ...
        bool haveMainFn = false;
        HirFunction main = new HirFunction();
        HirBlock mBlock = new HirBlock();
        main.name = "main";
        main.returnType = new PrimitiveType(BaseType.Void);
...
private:
    ...
    HirFunction lowerFunc(FuncDecl ast)
    {
        ...
        func.isOrnVarArgs = ast.isOrnVarArgs;
    ...
    HirNode lowerExpr(Node node)
    {
        ...
        switch (node.kind)
        {
            ...
            case NodeKind.LongLit:
                return new HirLongLit((cast(LongLit)node).value.get!long, node.resolvedType);
    ...
    HirNode lowerCall(CallExpr ast)
    {
        ...
        call.isVarArgAt = ast.isVarArgAt;
        call.isOrnVarArgs = ast.isOrnVarArgs;
    ...
    {
        int size = calculateTypeSize(ast.resolvedType_);
